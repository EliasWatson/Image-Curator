...
var $author$project$ImageCurator$GotImages = function (a) {
	return {$: 'GotImages', a: a};
};
var $author$project$ImageCurator$apiUrl = 'http://192.168.0.152:8080';
...
var $author$project$ImageCurator$initialCmd = $elm$http$Http$get(
	{
		expect: A2(
			...
			$author$project$ImageCurator$GotImages,
			$elm$json$Json$Decode$array($author$project$ImageCurator$imageDecoder)),
		url: $author$project$ImageCurator$apiUrl + '/get_images'
	});
var $author$project$ImageCurator$GotTexture = function (a) {
...
var $author$project$ImageCurator$getCurrentImage = function (model) {
	...
	switch (_v0.$) {
		case 'Loaded':
			...
			return A2(
				...
				$author$project$ImageCurator$emptyImage,
				A2($elm$core$Array$get, model.currentImageIndex, images));
		case 'Loading':
...
var $author$project$ImageCurator$updateCurrentTextureSource = function (model) {
	return _Utils_update(
		model,
		{
			currentTextureSource: A2(
				$joakin$elm_canvas$Canvas$Texture$loadFromImageUrl,
				$author$project$ImageCurator$apiUrl + ('/get_image/' + $author$project$ImageCurator$getCurrentImage(model).filename),
				$author$project$ImageCurator$GotTexture)
		});
};
var $author$project$ImageCurator$updateCurrentImage = F2(
	function (model, index) {
		var _v0 = model.status;
		switch (_v0.$) {
			case 'Loaded':
				var images = _v0.a;
				return $author$project$ImageCurator$updateCurrentTextureSource(
					_Utils_update(
						model,
						{
							currentImageIndex: $elm$core$Array$isEmpty(images) ? 0 : A3(
								$elm$core$Basics$clamp,
								0,
								$elm$core$Array$length(images) - 1,
								index)
						}));
			case 'Loading':
				return model;
...
var $author$project$ImageCurator$updateCurrentImageProperties = F2(
		...
		switch (_v0.$) {
			case 'Loaded':
				...
				return _Utils_update(
					model,
					{
						status: $author$project$ImageCurator$Loaded(
							A3($elm$core$Array$set, model.currentImageIndex, image, images))
					});
			case 'Loading':
				return model;
			default:
				...
				if (msg.a.$ === 'Ok') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								currentTextureSource: A2(
									$joakin$elm_canvas$Canvas$Texture$loadFromImageUrl,
									$author$project$ImageCurator$apiUrl + ('/get_image/' + A2(
										$elm$core$Maybe$withDefault,
										$author$project$ImageCurator$emptyImage,
										A2($elm$core$Array$get, 0, images)).filename),
									$author$project$ImageCurator$GotTexture),
								status: $author$project$ImageCurator$Loaded(images)
							}),
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								currentTextureSource: A2($joakin$elm_canvas$Canvas$Texture$loadFromImageUrl, 'img/error.png', $author$project$ImageCurator$GotTexture),
								status: $author$project$ImageCurator$Errored('Failed to load images')
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 'GotTexture':
				if (msg.a.$ === 'Nothing') {
					...
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								currentTexture: $author$project$ImageCurator$Errored('Failed to load texture')
							}),
						$elm$core$Platform$Cmd$none);
				} else {
					var texture = msg.a.a;
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				{
					height: $author$project$ImageCurator$canvasSize,
...
var $author$project$ImageCurator$SetCropSize = function (a) {
	return {$: 'SetCropSize', a: a};
};
var $author$project$ImageCurator$SetCropTop = function (a) {
...
var $author$project$ImageCurator$NoOp = {$: 'NoOp'};
var $author$project$ImageCurator$stopKeyPropagation = A2(
	$elm$html$Html$Events$stopPropagationOn,
	'keydown',
	$elm$json$Json$Decode$succeed(
...
var $author$project$ImageCurator$viewButton = F3(
	function (classes, msg, content) {
		return A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
					...
					A2(
						...
						A2(
							$elm$core$List$map,
							function (class_) {
								return _Utils_Tuple2(class_, true);
							},
							classes))),
...
var $author$project$ImageCurator$viewCheckbox = F3(
	function (msg, checked_, name) {
		return A2(
			$elm$html$Html$label,
			_List_Nil,
			_List_fromArray(
...
var $author$project$ImageCurator$NextImage = {$: 'NextImage'};
var $author$project$ImageCurator$PrevImage = {$: 'PrevImage'};
var $author$project$ImageCurator$SetCurrentImageStr = function (a) {
	return {$: 'SetCurrentImageStr', a: a};
};
var $elm$html$Html$i = _VirtualDom_node('i');
var $elm$html$Html$Attributes$placeholder = $elm$html$Html$Attributes$stringProperty('placeholder');
var $author$project$ImageCurator$viewNavigationBar = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				...
				_List_Nil,
				$author$project$ImageCurator$PrevImage,
				A2(
					$elm$html$Html$i,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('fas fa-backward')
						]),
					_List_Nil)),
				A2(
				$elm$html$Html$input,
				_List_fromArray(
					[
						...
						$elm$html$Html$Attributes$placeholder('Image Index'),
						$elm$html$Html$Attributes$value(
						$elm$core$String$fromInt(model.currentImageIndex + 1)),
						$elm$html$Html$Events$onInput($author$project$ImageCurator$SetCurrentImageStr),
						$author$project$ImageCurator$stopKeyPropagation
					]),
				_List_Nil),
				A3(
				$author$project$ImageCurator$viewButton,
				_List_Nil,
				$author$project$ImageCurator$NextImage,
				A2(
					$elm$html$Html$i,
					_List_fromArray(
						[
							$elm$html$Html$Attributes$class('fas fa-forward')
						]),
					_List_Nil))
			]));
};
var $author$project$ImageCurator$viewProperties = function (model) {
	var currentImage = $author$project$ImageCurator$getCurrentImage(model);
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
		...
		_List_fromArray(
			[
				...
				$author$project$ImageCurator$getCurrentImage(model).approved,
				'Approved'),
				A2(
				$elm$html$Html$table,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('crop-table')
					]),
				_List_fromArray(
					[
						A2(
				...
				$author$project$ImageCurator$viewNavigationBar(model)
			]));
};
...
var $author$project$ImageCurator$view = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
		...
		_List_fromArray(
			[
				$author$project$ImageCurator$viewWorkspace(model)
			]));
};
var $author$project$ImageCurator$main = $elm$browser$Browser$element(
	{
		init: function (_v0) {
			return _Utils_Tuple2($author$project$ImageCurator$initialModel, $author$project$ImageCurator$initialCmd);
		},
		...
		update: $author$project$ImageCurator$update,
		view: $author$project$ImageCurator$view
	});
_Platform_export({'ImageCurator':{'init':$author$project$ImageCurator$main(
